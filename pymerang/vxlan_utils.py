...
def create_or_update_fdb_entry(dst, lladdr, dev, port=VXLAN_DSTPORT):
    try:
        logging.debug(
            'Trying to create the FDB entry, dst %s, lladdr %s, dev %s, '
            'port %s',
            dst,
            lladdr,
            dev,
...
class TunnelVXLAN(tunnel_utils.TunnelMode):
    ...
    def create_tunnel_device_endpoint(self, deviceid, tenantid, vxlan_port):
        ...
        logging.debug(
            'Attempting to create the VXLAN '
            'interface %s with VNI %s',
            vxlan_name,
            vni
        )
        create_vxlan(
            ...
            vni=vni,
            dstport=vxlan_port,
            srcport_min=vxlan_port,
            srcport_max=vxlan_port+1,
            udpcsum=ENABLE_UDP_CSUM,
    ...
    def create_tunnel_device_endpoint_end(
        ...
        logging.debug(
            'Attempting to add the entry to the FDB\n'
        ...
        logging.debug(
            'Attempting to add the neigh to the neigh table\n'
            'dst=%s, lladdr=%s, vxlan_name=%s',
            controller_vtep_ip,
            controller_vtep_mac,
            vxlan_name
        )
        tunnel_utils.create_ip_neigh(
            dev=vxlan_name,
            lladdr=controller_vtep_mac,
    ...
    def create_tunnel_controller_endpoint(
        ...
        logging.info(
            'Configuring the VXLAN tunnel for the device %s', deviceid
        )
        ...
        storage_helper.update_device_vtep_mac(
        ...
        if family == socket.AF_INET6:
            ip_mask = storage_helper.get_new_mgmt_ipv6('0').split('/')
            controller_vtep_ip = ip_mask[0]
            ip_mask = storage_helper.get_new_mgmt_ipv6(deviceid).split('/')
            device_vtep_ip = ip_mask[0]
            vtep_mask = int(ip_mask[1])
        elif family == socket.AF_INET:
            ip_mask = storage_helper.get_new_mgmt_ipv4('0').split('/')
            controller_vtep_ip = ip_mask[0]
            ip_mask = storage_helper.get_new_mgmt_ipv4(deviceid).split('/')
            device_vtep_ip = ip_mask[0]
            vtep_mask = int(ip_mask[1])
        else:
            logging.error(
        ...
        create_or_update_fdb_entry(
            dev=vxlan_name,
            lladdr=device_vtep_mac,
            dst=device_external_ip,
        ...
        controller_vtep_mac = tunnel_utils.get_mac_address(ifname=vxlan_name)
        # Update device VTEP IP address
        success = storage_helper.update_device_vtep_ip(
    ...
    def destroy_tunnel_device_endpoint_end(self, deviceid, tenantid,
        ...
        # of the controller to the VTEP MAC address
        logging.debug(
            'Attempting to remove the neigh from the neigh table\n'
            'dst=%s, lladdr=%s, vxlan_name=%s',
            controller_vtep_ip,
            controller_vtep_mac,
            vxlan_name
        )
        tunnel_utils.remove_ip_neigh(dev=vxlan_name, dst=controller_vtep_ip)
        ...
        if device_vtep_ip is not None:
            ...
            tunnel_utils.del_address(
                device=vxlan_name, address=device_vtep_ip, mask=vtep_mask
            )
    ...
    def destroy_tunnel_device_endpoint(self, deviceid, tenantid):
        logging.info(
            'Destroying the VXLAN tunnel (destroy_tunnel_device_endpoint)'
        )
        # VNI used for VXLAN management interface
        vni = MGMT_VNI
        # Name of the VXLAN interface
        ...
        logging.debug(
            'Attempting to remove the VXLAN interface %s', vxlan_name
        )
    ...
    def destroy_tunnel_controller_endpoint(self, deviceid, tenantid):
        ...
        # of the device to the device VTEP MAC address
        device_vtep_ip = storage_helper.get_device_mgmtip(
            tenantid, deviceid
        ).split('/')[0]
        logging.debug(
            'Attempting to remove the neigh from the neigh table\n'
        ...
        try:
            tunnel_utils.remove_ip_neigh(dev=vxlan_name, dst=device_vtep_ip)
        ...
        # to the device IP address
        device_vtep_mac = storage_helper.get_device_vtep_mac(
            deviceid, tenantid
        )
        logging.debug(
            'Attempting to remove the entry from the FDB\n'
        ...
        storage_helper.release_ipv4_address(
            deviceid, tenantid
        )  # TODO error check
        storage_helper.release_ipv6_address(deviceid, tenantid)
        # Success
        logging.debug('The VXLAN interface has been removed')
        return status_codes_pb2.STATUS_SUCCESS
    ...
    def update_tunnel_device_endpoint(
        ...
        if res == status_codes_pb2.STATUS_SUCCESS:
            res = self.create_tunnel_device_endpoint(
                deviceid,
                tenantid,
                controller_vtep_ip,
                device_vtep_ip,
    ...
    def update_tunnel_device_endpoint_end(
        ...
        return self.create_tunnel_device_endpoint_end(
            deviceid,
            tenantid,
            controller_vtep_ip,
            device_vtep_ip,
    ...
    def update_tunnel_controller_endpoint(
        ...
        res = self.destroy_tunnel_controller_endpoint(deviceid, tenantid)
        if res == status_codes_pb2.STATUS_SUCCESS:
    ...
    def init_tunnel_controller_endpoint(self):
        ...
        # ip_mask = storage_helper.get_new_mgmt_ipv6(
        #    '0', tenantid).split('/')
        # controller_vtep_ipv6 = ip_mask[0]
        # vtep_mask_ipv6 = int(ip_mask[1])
        # ip_mask = storage_helper.get_new_mgmt_ipv4(
        #    '0', tenantid).split('/')
        ...
        # Create the VXLAN interface
        logging.debug(
            'First VXLAN tunnel, attempting to create '
    ...
    def destr_tunnel_controller_endpoint(self):
        ...
        ip_mask = storage_helper.get_device_mgmtipv4(tenantid, '0').split('/')
        controller_vtep_ipv4 = ip_mask[0]
        ...
        # Remove the address from the VTEP interface
        ip_mask = storage_helper.get_device_mgmtipv6(tenantid, '0').split('/')
        controller_vtep_ipv6 = ip_mask[0]
        ...
        logging.debug(
            'Last VXLAN tunnel, attempting to remove '
            'the VXLAN interface %s',
            vxlan_name
        )
